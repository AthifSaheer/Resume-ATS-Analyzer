...
from crud import create_company_track
...
if st.button("Submit", type="primary"):
    ...
    with st.spinner("Analyzing with LLM..."):
        suggestions = analyze_with_llm(resume_text, jd, custom_prompt)

    # ── Save to DB ────────────────────────────────────────────────────────
    record_id = create_company_track(
    ...
    st.success(f"✅ Saved to Company Tracker (ID: {record_id})")

    # ── Render AI response ────────────────────────────────────────────────
    def render_json_ui(ai_response):
        try:
            data = ai_response if isinstance(ai_response, dict) else json.loads(ai_response)
        except Exception:
